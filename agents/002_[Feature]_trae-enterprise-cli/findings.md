...
- [实现] standalone model-auth entry 始终显示 Trae Enterprise CLI；仅 probe status 的 available 为 true 时才可选择，已保存 session 仅在 authenticated 时健康。共享 adapter 现在以 `assistantText` 返回推理结果，不解析 raw stdout 或未稳定 JSONL 工具事件。
