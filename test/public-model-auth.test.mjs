...
const release = "https://github.com/lsy-404/platform-kit/releases/download/v0.6.1";
for (const name of ["core", "providers"]) {
  assert.equal(manifest.dependencies[`@model-auth/${name}`], `${release}/model-auth-${name}-0.6.1.tgz`);
  assert.match(lock, new RegExp(`${release.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/model-auth-${name}-0\\.6\\.1\\.tgz`));
}
const vueRelease = "https://github.com/lsy-404/platform-kit/releases/download/v0.6.1/model-auth-vue-0.6.1.tgz";
